...
const generateTranslationContent = (localeCode, key) => {
    const templates = TRANSLATION_TEMPLATES[localeCode] ||
...
async function seedEnterpriseAccounts() {
    ...
    for (const enterpriseData of enterprises) {
        ...
        if (!existingEnterprise) {
            ...
            console.log(`Created enterprise: ${enterprise.name} (${enterprise.platform})`);
        }
        else {
            enterprise = existingEnterprise;
            console.log(`Enterprise already exists: ${enterprise.name}`);
        }
        const enterpriseUsersForThisEnterprise = enterpriseUsers.filter((u) => u.provider === enterprise.platform);
...
async function seedTestData() {
    ...
    for (const project of projects) {
        await createProjectTestData(project, adminUser.id, editorUser?.id, reviewerUser?.id);
    }
    console.log('Test data seeding finished.');
}
async function createTestProjects(adminUserId, editorUserId) {
...
async function createProjectTestData(project, adminUserId, editorUserId, reviewerUserId) {
    ...
    const baseLocaleId = projectLocales.find((pl) => pl.Locale.code === project.baseLocale)?.Locale.id;
    if (!baseLocaleId) {
        ...
        for (const keyDef of keys) {
            ...
            if (!key) {
                key = await prisma.key.create({
                    data: {
                        ...
                        type: keyDef.type || client_1.KeyType.TEXT,
                        namespaceId: namespace.id,
                    },
                });
            }
            for (const localeId of localeIds) {
                ...
                if (!existingTranslation) {
                    const locale = projectLocales.find((pl) => pl.Locale.id === localeId)?.Locale;
                    ...
                    let approvedAt = null;
                    if (isBaseLocale) {
                        status = client_1.TranslationStatus.PUBLISHED;
                        content = keyDef.defaultValue || keyDef.name;
                    }
                    else {
                        const rand = Math.random();
                        if (rand < 0.2) {
                            status = client_1.TranslationStatus.PENDING;
                            content = '';
                        }
                        else if (rand < 0.4) {
                            status = client_1.TranslationStatus.TRANSLATING;
                            content = generateTranslationContent(locale?.code || 'en-US', keyDef.name);
                            submitterId = editorUserId || adminUserId;
                        }
                        else if (rand < 0.6) {
                            status = client_1.TranslationStatus.REVIEWING;
                            content = generateTranslationContent(locale?.code || 'en-US', keyDef.name);
                            submitterId = editorUserId || adminUserId;
                        }
                        else if (rand < 0.8) {
                            status = client_1.TranslationStatus.APPROVED;
                            content = generateTranslationContent(locale?.code || 'en-US', keyDef.name);
                            submitterId = editorUserId || adminUserId;
                    ...
                    await prisma.translation.create({
                        data: {
                            ...
                            priority: Math.random() > 0.7
                                ? client_1.TranslationPriority.HIGH
                                : client_1.TranslationPriority.MEDIUM,
...
function generateKeysForNamespace(namespace) {
    const keysMap = {
        common: [

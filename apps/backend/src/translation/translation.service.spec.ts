...
describe('TranslationService', () => {
  ...
  beforeEach(() => {
    prisma.locale.findUnique.mockResolvedValue({ id: localeId, code: localeCode });
    prisma.key.findFirst.mockResolvedValue({ id: keyId, namespaceId, namespace: { projectId } });
  });
  ...
  describe('create', () => {
    ...
    it('should create a translation', async () => {
      const expectedResult = { id: 'trans-1', ...createDto, keyId, localeId, status: TranslationStatus.PENDING, isLlmTranslated: false };
      prisma.translation.create.mockResolvedValue(expectedResult);

      const result = await service.create(projectId, namespaceId, keyId, createDto);
      expect(result).toEqual(expectedResult);
      expect(prisma.locale.findUnique).toHaveBeenCalledWith({ where: { code: localeCode } });
      expect(prisma.key.findFirst).toHaveBeenCalledWith({ where: { id: keyId, namespaceId, namespace: { projectId } } });
    ...
    it('should throw NotFoundException if locale not found', async () => {
      ...
      await expect(service.create(projectId, namespaceId, keyId, createDto)).rejects.toThrow(NotFoundException);
    });
  ...
  describe('findAll', () => {
    it('should return all translations for a key', async () => {
      const expectedResult = [{ id: 'trans-1', content: 'Hello', keyId, localeId }];
  ...
  describe('findOne', () => {
    it('should return a single translation', async () => {
      ...
      const result = await service.findOne(projectId, namespaceId, keyId, localeCode);
      expect(result).toEqual(expectedResult);
      expect(prisma.locale.findUnique).toHaveBeenCalledWith({ where: { code: localeCode } });
    ...
    it('should throw NotFoundException if translation not found', async () => {
      ...
      await expect(service.findOne(projectId, namespaceId, keyId, localeCode)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('update', () => {
    ...
    it('should update a translation', async () => {
      const expectedResult = { id: 'trans-1', content: 'Hi there', keyId, localeId };
      prisma.translation.update.mockResolvedValue(expectedResult);

      const result = await service.update(projectId, namespaceId, keyId, localeCode, updateDto);
      expect(result).toEqual(expectedResult);
      expect(prisma.locale.findUnique).toHaveBeenCalledWith({ where: { code: localeCode } });
      expect(prisma.translation.update).toHaveBeenCalledWith({
        ...
        data: {
          ...
          isLlmTranslated: updateDto.isLlmTranslated,
        },
      });
    });
    ...
    it('should throw NotFoundException if translation to update is not found', async () => {
      ...
      await expect(service.update(projectId, namespaceId, keyId, localeCode, updateDto)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('approve', () => {
    it('should approve a translation', async () => {
      ...
      const result = await service.approve(projectId, namespaceId, keyId, localeCode);
      expect(result).toEqual(expectedResult);
      expect(prisma.translation.update).toHaveBeenCalledWith({
        where: { keyId_localeId: { keyId, localeId } },
  ...
  describe('reject', () => {
    it('should reject a translation', async () => {
      ...
      const result = await service.reject(projectId, namespaceId, keyId, localeCode);
      expect(result).toEqual(expectedResult);
      expect(prisma.translation.update).toHaveBeenCalledWith({
        where: { keyId_localeId: { keyId, localeId } },
  ...
  describe('publish', () => {
    it('should publish an approved translation', async () => {
      const approvedTranslation = { id: 'trans-1', status: TranslationStatus.APPROVED };
      const expectedResult = { ...approvedTranslation, status: TranslationStatus.PUBLISHED };
      prisma.translation.findUnique.mockResolvedValue(approvedTranslation);
      prisma.translation.update.mockResolvedValue(expectedResult);

      const result = await service.publish(projectId, namespaceId, keyId, localeCode);
      expect(result).toEqual(expectedResult);
      expect(prisma.translation.findUnique).toHaveBeenCalledWith({ where: { keyId_localeId: { keyId, localeId } } });
      expect(prisma.translation.update).toHaveBeenCalledWith({
        where: { keyId_localeId: { keyId, localeId } },
    ...
    it('should throw BadRequestException if translation is not approved', async () => {
      const pendingTranslation = { id: 'trans-1', status: TranslationStatus.PENDING };
      prisma.translation.findUnique.mockResolvedValue(pendingTranslation);

      await expect(service.publish(projectId, namespaceId, keyId, localeCode)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('remove', () => {
    it('should delete a translation', async () => {
      ...
      const result = await service.remove(projectId, namespaceId, keyId, localeCode);
      expect(result).toEqual({ success: true });
      expect(prisma.translation.delete).toHaveBeenCalledWith({ where: { keyId_localeId: { keyId, localeId } } });
    });
    ...
    it('should throw NotFoundException if translation to delete is not found', async () => {
      ...
      await expect(service.remove(projectId, namespaceId, keyId, localeCode)).rejects.toThrow(NotFoundException);
    });
  });
});
